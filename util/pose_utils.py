...
def draw_pose_from_cords(pose_joints, img_size=(256,256), radius=2, draw_joints=True):
    colors = np.zeros(shape=img_size + (3, ), dtype=np.uint8)
    mask = np.zeros(shape=img_size, dtype=bool)

    pose_joints = pose_joints.astype(np.uint8)
    if draw_joints:
        for f, t in LIMB_SEQ:
            ...
            from_missing = pose_joints[f][0] == MISSING_VALUE2 or pose_joints[f][1] == MISSING_VALUE2
            to_missing = pose_joints[t][0] == MISSING_VALUE2 or pose_joints[t][1] == MISSING_VALUE2
